...
mcp.tool({
  ...
  handler: async ({ from_date, to_date }: any) => {
    let q = svc().from("activity_rows").select("cost_total").limit(50000);
...
mcp.tool({
  ...
  handler: async ({ from_date, to_date, limit = 20 }: any) => {
    let q = svc().from("activity_rows").select("model_permaslug, cost_total, tokens_prompt, tokens_completion").limit(50000);
    if (from_date) q = q.gte("created_at", from_date);
    if (to_date) q = q.lte("created_at", to_date + "T23:59:59");
    const { data } = await q;
...
mcp.tool({
  ...
  handler: async () => {
    const { data } = await svc().from("activity_rows").select("provider_name, cost_total").limit(50000);
...
mcp.tool({
  ...
  handler: async ({ limit = 20 }: any) => {
    const { data } = await svc().from("activity_rows").select("*").order("created_at", { ascending: false }).limit(Math.min(limit, 100));
...
mcp.tool({
  ...
  inputSchema: { type: "object", properties: {} },
  handler: async () => {
    const { data } = await svc().from("activity_rows").select("model_permaslug, provider_name").limit(50000);
...
mcp.tool({
  name: "delete_request",
  description: "Smaže záznam podle generation_id (write operace).",
  inputSchema: { type: "object", properties: { generation_id: { type: "string" } }, required: ["generation_id"] },
  handler: async ({ generation_id }: any) => {
    const { error } = await svc().from("activity_rows").delete().eq("generation_id", generation_id);
...
mcp.tool({
  ...
  description: "POZOR: Smaže VŠECHNA data uživatele. Vyžaduje confirm=true.",
  ...
  handler: async ({ confirm }: any) => {
    ...
    const { error, count } = await svc().from("activity_rows").delete({ count: "exact" }).neq("generation_id", "");
